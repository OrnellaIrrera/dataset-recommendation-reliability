...
dataframe = open('../../data/datafinder/Abstract_New_Database.txt')
# dataframe = open("Citation_New_Database.txt", encoding='ISO-8859-1')


documentation_file_parameteropt = open("path/to/results/mes/parameter_optimization_tfidf_datafinder_finale1.txt", "w+")
documentation_file_modelopt = open(".path/to/results/mes/classifier_optimization_tfidf_datafinder_finale1.txt", "w+")

titles = []
with open("path/to/data/datafinder/Dataset_Titles.txt") as titles_file:
...
if os.path.exists('path/to/data/tmp/mes/aux_store_data.json'):
    ...
    for i, line in enumerate(dataframe):
        ...
        g = open('path/to/data/tmp/datafinder/tmp_data_2.json', 'w')
        json.dump(
            {'queries_id': query_ids, 'datasets': datasets, 'datasets_single': datasets_single, 'queries': queries,
...
# the first 17938 vectors are for training, the remaining for test
json_data = {}
json_data['training'] = {}
json_data['test'] = {}
json_data['training']['queries'] = q_train = q_tfidf[0:17398]
json_data['training']['query_ids'] = query_ids[0:17398]
json_data['training']['datasets'] = d_train = datasets[0:17398]
json_data['test']['queries'] = q_test = q_tfidf[17398:]
json_data['test']['query_ids'] = query_ids[17398:]
json_data['test']['datasets'] = d_test = datasets[17398:]
print(q_tfidf.shape)


# g = open('../../data/tmp/mes/split.json','r')
