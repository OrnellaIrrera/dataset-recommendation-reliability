...
def graphwalk(G: nx.Graph, hop: int, seed, cand_list):
    walk_authors = set()
    if G.has_node(seed.replace("http://example.org/", "")):
        temp_set = set(
            nx.single_source_shortest_path(G, seed.replace("http://example.org/", ""), cutoff=(2 * hop)).keys())
        temp_set = set([str("http://example.org/" + t) for t in temp_set])
...
def read_vec_dict(path):
    ...
    with open(path) as rfile:
        for line in rfile:
            spline = line.split()
            if spline[1].startswith("http://example.org/"):
                vec_dict[str(spline[1].replace("http://example.org/", ""))] = int(spline[0])
    rfile.close()
    return vec_dict
...
def authors_to_embed_dict(author_list, vec_dict, fp):
    ...
    for author in author_list:
        try:
            embed_dict[author] = fp[vec_dict[author.replace("http://example.org/", "")]]
        except:
            print('passing',author)
            pass
    return embed_dict
...
def loop_build_graph(G: nx.Graph, step: int, node, coauthor_hdt):
    if step % 2 == 0:
        ...
        for sub, _, obj in triples:
            ...
            G.add_edge(s.replace("http://example.org/", ""), o.replace("http://example.org/", ""))
            step += 1
            if step < 3:
...
if __name__ == '__main__':
    ...
    fp = np.memmap(vec_path, mode='r', dtype='float32', shape=(31109, 100))
